# app.py – option‑chain screener (monthly) + Telegram alerts
# ───────────────────────────────────────────────────────────
...
WIDTH   = 2                      # ATM ±2 strikes to examine

# ─── File locations (works locally & on Render disk) ───────
DATA_DIR     = os.getenv("DATA_DIR", ".")          # change to "/data" if you attach a Render disk
...
TELEGRAM_TOKEN   = os.getenv("TELEGRAM_TOKEN")      # e.g. 7955…:AA…
TELEGRAM_CHAT_ID = os.getenv("TELEGRAM_CHAT_ID")    # e.g. 7983…
...
def next_expiry(symbol: str):
    ...
    for d in dates:
        if d >= today:
            return d
    return dates[-1]      # fallback (should rarely hit)
    ...
    try:
        ...
        if not strikes:
            ...
            for st in strikes:
                ...
                put_tags.append (
                    f"{st}{check_option(pe,  True) if pe else '❌'}")
                call_tags.append(
                    f"{st}{check_option(ce, False) if ce else '❌'}")
        ...
        # Send Telegram only if at least one ✅ appears
